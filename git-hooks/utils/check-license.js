 ...
 * License - Apache 2.0
 * Copyright 2024 Faculty Science Limited
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
...
let checkLicenseUtil = {
  ...
  checkLicense: function(lines, licenseHeaderLines) {
    ...
    for (let i = 0; i < licenseHeaderLines.length - 1; i++) {
      ...
      // console.log(lines[i + 2], licenseHeaderLines[i])
      let isContainedInLicenseHEader = lines[i + 2].indexOf(licenseHeaderLines[i]) > -1;
      list.push(isContainedInLicenseHEader);
    }

    let result = list.every(val => val);
    return result
  },
